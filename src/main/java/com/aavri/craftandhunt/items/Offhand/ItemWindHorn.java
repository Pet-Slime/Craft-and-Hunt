...
public class ItemWindHorn extends Item {

	public ItemWindHorn(Properties properties) {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        } else if (offhandSlot.getItem().equals(RegisterOffhand.wind_horn.get())) {
    ...
    public void castSpell(World worldIn, PlayerEntity playerIn) {
		...
		for(LivingEntity X: target) {
			...
			if (X != playerIn) {
				X.applyKnockback(2.0F, x - X.getPosX(), z - X.getPosZ());
			}
		}
